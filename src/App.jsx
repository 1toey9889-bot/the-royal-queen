...
export default function App() {
  ...
  // 📊 Dashboard (New Orders Only)
  const DashboardView = () => {
    const [filterStore, setFilterStore] = useState('all');
    const filtered = useMemo(() => orders.filter(o => filterStore === 'all' || o.store === filterStore), [orders, filterStore]);
  ...
  const SalesView = () => {
    ...
    const confirmOrder = async () => {
      if(!orderId.trim() || cart.length === 0) return alert('กรุณาใส่เลขบิลและเลือกสินค้า');
      setProcessing(true);
      try {
        await runTransaction(db, async (t) => {
          const oRef = doc(db, "orders", orderId.trim()); if((await t.get(oRef)).exists()) throw new Error("เลขบิลซ้ำ");
          cart.forEach(i => { t.update(doc(db,"products",i.pid), { stock: increment(-i.qty) }); });
